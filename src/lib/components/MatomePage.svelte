<script lang="ts">
  ...
  // $: で naddr プロップを監視する（user/[npub] と同じパターン）。
  let _initedNaddr: string | null = null;
  function initForNaddr(addr: string): void {
    if (!browser) return;
    if (addr === _initedNaddr) return;
    _initedNaddr = addr;
    ...
    try {
      const decoded = nip19.decode(addr) as { type: string; data: unknown };
  ...
  $: authorStyle = matome ? avatarStyle(matome.pubkey, authorName) : { bg: '#e5e5e5', fg: '#737373', initial: '?' };
